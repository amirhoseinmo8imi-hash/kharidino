/* Kharidino mobile marketplace UX */
(function () {
  'use strict';
  const mobile = () => window.matchMedia('(max-width: 768px)').matches;

  function enhanceHome() {
    if (!mobile() || !document.querySelector('.product-grid')) return;
    ...
    // Turn the category row into a native-feeling horizontal rail.
    const categoryGrid = document.querySelector('.category-grid');
    if (categoryGrid) categoryGrid.classList.add('mobile-category-rail');

    // Add a compact quick-filter rail above the product feed.
    ...
    if (section && !section.querySelector('.mobile-quick-filters')) {
      ...
      filter.addEventListener('click', openFilterSheet);
    }
  }
  ...
  function openFilterSheet() {
    ...
    sheet.innerHTML = '<div class="mobile-sheet-backdrop"></div>' +
      '<div class="mobile-sheet-panel" role="dialog" aria-modal="true">' +
      '<div class="mobile-sheet-head"><strong>فیلتر و مرتب‌سازی</strong><button type="button" class="sheet-close"><i class="fa-solid fa-xmark"></i></button></div>' +
      '<div class="mobile-sheet-body"></div>' +
      '<div class="mobile-sheet-actions"><button type="button" class="sheet-reset">پاک کردن</button><button type="button" class="sheet-apply">اعمال فیلتر</button></div></div>';
    ...
    ['input[name="q"]','select[name="category"]','select[name="sort"]'].forEach(sel => {
      ...
      if (el) {
        ...
        clone.classList.add('sheet-control');
