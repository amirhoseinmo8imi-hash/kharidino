...
def _make_flow(db, User, Store, Order, SellerOrder, SellerLedger, user, store, *, ledger_status="available"):
...
def test_refund_before_settlement_has_no_clawback_and_is_idempotent():
    ...
    with app.app_context():
        ...
        store = Store(name="Refund Before Store", active=True)
        db.session.add_all([user, store]); db.session.flush()
        order, seller_order, ledger, tx = _make_flow(db, User, Store, Order, SellerOrder, SellerLedger, user, store, ledger_status="pending")
        _refund(app, user, tx)
...
def test_refund_during_requested_settlement_cancels_reservation():
    ...
    with app.app_context():
        ...
        store = Store(name="Requested Refund Store", active=True)
        db.session.add_all([user, store]); db.session.flush()
        order, seller_order, ledger, tx = _make_flow(db, User, Store, Order, SellerOrder, SellerLedger, user, store)
        ...
        db.session.delete(db.session.get(RefundRecord, RefundRecord.query.filter_by(payment_transaction_id=tx.id).one().id)); db.session.delete(db.session.get(PaymentTransaction, tx.id)); db.session.delete(settlement); db.session.delete(db.session.get(SellerLedger, ledger.id)); db.session.delete(db.session.get(SellerOrder, seller_order.id)); db.session.delete(db.session.get(Order, order.id)); db.session.delete(store); db.session.delete(user); db.session.commit()


def test_partial_then_full_clawback_reconciles_exactly_once():
    app, db, Order, Store, User, SellerLedger, SellerOrder, SellerSettlement, SellerSettlementAllocation = _boot()
    from accounting import SellerSettlementAllocation
    SellerClawback = app.extensions["kharidino_seller_clawback"]
    ...
    with app.app_context():
        ...
        store = Store(name="Clawback Store", active=True)
        db.session.add_all([user, store]); db.session.flush()
        order, seller_order, ledger, tx = _make_flow(db, User, Store, Order, SellerOrder, SellerLedger, user, store, ledger_status="paid")
        ...
        db.session.delete(clawback); db.session.delete(refund); db.session.delete(db.session.get(PaymentTransaction, tx.id)); db.session.delete(settlement); db.session.delete(db.session.get(SellerSettlementAllocation, SellerSettlementAllocation.query.filter_by(settlement_id=settlement.id).first().id)) if SellerSettlementAllocation.query.filter_by(settlement_id=settlement.id).first() else None
        db.session.delete(db.session.get(SellerLedger, ledger.id)); db.session.delete(db.session.get(SellerOrder, seller_order.id)); db.session.delete(db.session.get(Order, order.id)); db.session.delete(store); db.session.delete(user); db.session.commit()
...
def test_database_guard_rejects_cross_settlement_double_allocation():
    ...
    with app.app_context():
        ...
        store = Store(name="Allocation Guard Store", active=True)
        db.session.add_all([user, store]); db.session.flush()
        order, seller_order, ledger, tx = _make_flow(db, User, Store, Order, SellerOrder, SellerLedger, user, store)
        ...
        db.session.add(SellerSettlementAllocation(settlement_id=first.id, ledger_id=ledger.id, amount=60_000)); db.session.commit()
        ...
        db.session.query(SellerSettlement).filter(SellerSettlement.id.in_([first.id, second.id])).delete(synchronize_session=False)
        db.session.delete(db.session.get(PaymentTransaction, tx.id)); db.session.delete(db.session.get(SellerLedger, ledger.id)); db.session.delete(db.session.get(SellerOrder, seller_order.id)); db.session.delete(db.session.get(Order, order.id)); db.session.delete(store); db.session.delete(user); db.session.commit()
