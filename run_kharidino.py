...
register_mobile_api(app, db, Product, Category, Store, Offer, User)
...
def phone_connection():
    ...
    return ("<!doctype html><html lang='fa' dir='rtl'><meta charset='utf-8'>"
            ...
            f"<div class='card'><h1>📱 اتصال خریدینو به گوشی</h1><p class='ok'>سرور روی شبکه محلی فعال است.</p>"
            f"<span class='url'>http://{host}:{port}</span>"
            "<ol><li>گوشی و کامپیوتر را به یک Wi‑Fi وصل کن.</li><li>آدرس بالا را در Chrome گوشی وارد کن.</li>"
...
@app.before_request
def splash_gate():
    if (request.path.startswith("/static/") or request.path.startswith("/splash") or
        request.path.startswith("/phone") or request.path.startswith("/api/mobile/") or
        request.path.startswith("/admin/kharidino-ai/api/")):
        return None
...
def validate_runtime_config():
    ...
    if not secret or secret in {"change-this-secret-key", "dev-secret"}:
        if not debug:
            raise RuntimeError("SECRET_KEY is not configured. Set a strong random SECRET_KEY before starting Kharidino.")
    ...
    if not debug and os.environ.get("KHARIDINO_ADMIN_PASSWORD", "").strip() in {"", "admin12345"}:
        app.logger.warning("KHARIDINO_ADMIN_PASSWORD is not configured. Do not rely on a default administrator password.")
