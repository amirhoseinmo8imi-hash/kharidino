...
def csrf_token() -> str:
    """Return a stable per-session CSRF token, creating it when necessary."""
...
def apply_security(app):
    ...
    app.config["MAX_CONTENT_LENGTH"] = min(
        int(app.config.get("MAX_CONTENT_LENGTH") or _MAX_UPLOAD_BYTES),
        _MAX_UPLOAD_BYTES,
    )
...
def _check_csrf() -> None:
    """Require the session-bound CSRF token for browser state changes."""
...
def _validate_uploaded_files() -> None:
    """Validate actual file content, not only the user-controlled extension."""
