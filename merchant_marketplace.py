...
class SellerProduct(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("store_id", "product_id", name="uq_seller_product_store_product"),
    )
...
def _seller_account(user_id=None):
    ...
    if not user or user.role != "seller":
        return None
    account = MerchantStore.query.filter_by(user_id=user.id).first()
    return account
...
def _seller_products(store_id):
    links = (
        ...
        .order_by(SellerProduct.id.desc())
        .all()
    )
...
def seller_register():
    ...
    if request.method == "POST":
        ...
        store_name = _store_slug_name(request.form.get("store_name", ""))
        ...
        if User.query.filter_by(email=email).first():
            ...
            return render_template("seller_register.html")
        if not website.startswith(("http://", "https://")) and website:
...
def seller_dashboard():
    ...
    stats = {
        ...
        "offers": len(offers),
        "in_stock": sum(1 for o in offers if o.in_stock),
...
def seller_store_save():
    account = _seller_account()
    store = account.store
    name = _store_slug_name(request.form.get("name", ""))
    website = request.form.get("website", "").strip()
    if not name:
        flash("نام فروشگاه الزامی است.", "warning")
        return redirect(url_for("seller_dashboard"))
    if website and not website.startswith(("http://", "https://")):
        flash("لینک وب‌سایت نامعتبر است.", "warning")
        return redirect(url_for("seller_dashboard"))
    ...
    if uploaded and uploaded.filename:
        old = store.logo
        try:
            # save_image stores a random safe filename under products; for store logos
            # we keep the existing store upload helper in app.py via a local import.
...
def seller_product_save():
    ...
    stock = request.form.get("in_stock") == "1"
    url = request.form.get("url", "").strip()

    if not name:
        flash("نام محصول الزامی است.", "warning")
        return redirect(url_for("seller_dashboard"))
    try:
        price = max(0, int(price_raw or 0))
        category = db.session.get(Category, int(category_id)) if category_id else None
    except (TypeError, ValueError):
        flash("قیمت یا دسته‌بندی نامعتبر است.", "danger")
        return redirect(url_for("seller_dashboard"))
    if category_id and not category:
        flash("دسته‌بندی پیدا نشد.", "danger")
        return redirect(url_for("seller_dashboard"))
    if url and not url.startswith(("http://", "https://")):
        flash("لینک خرید نامعتبر است.", "danger")
        return redirect(url_for("seller_dashboard"))
    ...
    if pid:
        ...
        db.session.flush()
        db.session.add(SellerProduct(store_id=account.store_id, product_id=product.id))
    ...
    if not offer:
        offer = Offer(product_id=product.id, store_id=account.store_id)
        db.session.add(offer)
    offer.price = price
    offer.url = url
    offer.in_stock = stock
    db.session.commit()
    flash("محصول و قیمت فروشگاه ذخیره شد. ✅", "success")
...
def seller_product_delete(product_id):
    ...
    foreign_offers = Offer.query.filter(Offer.product_id == product.id, Offer.store_id != account.store_id).count()
    if foreign_offers:
        flash("این محصول در فروشگاه‌های دیگر هم استفاده شده و حذف کامل آن مجاز نیست. ابتدا فقط موجودی خودت را متوقف کن.", "warning")
        return redirect(url_for("seller_dashboard") + "#products")
    if product.image:
        remove_upload(product.image)
...
def seller_offer_toggle(product_id):
    ...
    if not offer:
        abort(404)
    offer.in_stock = not offer.in_stock
    db.session.commit()
...
def admin_merchants():
    ...
    return render_template("admin_merchants.html", accounts=accounts)
...
def admin_merchant_status(account_id):
    ...
    db.session.commit()
    flash("وضعیت فروشگاه به‌روزرسانی شد. ✅", "success")
...
def admin_merchant_delete(account_id):
    account = MerchantStore.query.get_or_404(account_id)
    account.store.active = False
    account.status = "rejected"
    db.session.commit()
    flash("فروشگاه غیرفعال شد.", "success")
    return redirect(url_for("admin_merchants"))
