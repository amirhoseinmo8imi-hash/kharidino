...
The existing checkout remains the source of truth for order creation. This wrapper
adds optional coupon and customer-wallet payment immediately after the order is
created, before the payment-start request can use the order total.
"""
...
from marketplace_hardening import _normalize_coupon, _tier_for_points
...
def _apply_adjustments(order, coupon_code, wallet_amount):
    ...
    if coupon_code:
        ...
        if existing:
            ...
            changed = db.session.query(Coupon).filter(
                Coupon.id == coupon.id,
                Coupon.active.is_(True),
                *([Coupon.used_count < coupon.usage_limit] if coupon.usage_limit is not None else []),
            ).update({Coupon.used_count: Coupon.used_count + 1}, synchronize_session=False)
            if changed != 1:
    ...
    wallet_used = 0
    if wallet_amount:
        requested = max(0, int(wallet_amount))
        requested = min(requested, subtotal_after_coupon)
        ...
        # Wallet debit is represented in the same transaction as the order total.
...
def apply_checkout_marketplace_adjustments(app_obj=None):
    ...
    if checkout_view and not getattr(checkout_view, "_kharidino_marketplace_adjustments", False):
        ...
        def checkout_with_marketplace_adjustments(*args, **kwargs):
            ...
            wallet_amount = 0
            if use_wallet or raw_wallet not in (None, ""):
                try:
                    wallet_amount = max(0, int(raw_wallet or 0))
                except (TypeError, ValueError):
            ...
            if len(new_orders) != 1 or (not coupon_code and not wallet_amount):
                return response

            order = new_orders[0]
            try:
                result = _apply_adjustments(order, coupon_code, wallet_amount)
                db.session.commit()
