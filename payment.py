...
class TestGateway(PaymentGateway):
    ...
    def start(self, transaction_id: str, amount: int, callback_url: str) -> GatewayStartResult:
        ...
        return GatewayStartResult("redirect", f"{callback_url}?transaction={transaction_id}&authority={authority}&test=1", authority)
    def verify(self, transaction_id: str, amount: int, payload: dict) -> GatewayVerifyResult:
        ...
        return GatewayVerifyResult(True, "TEST-" + hashlib.sha256(authority.encode()).hexdigest()[:20])
...
def apply_payment(app, db, Order, User):
    ...
    def payment_start(order_id):
        ...
        if not order or order.user_id != session["user_id"]:
            abort(404)
        if order.status == "لغو شد" or int(order.total or 0) <= 0:
            abort(409, description="سفارش قابل پرداخت نیست.")
        try:
        ...
        result = _gateway().start(tx.public_id, tx.amount, url_for("payment_callback", transaction_id=tx.public_id, _external=True))
